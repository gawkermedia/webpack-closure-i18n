...
function buildI18n(fileContents, translations, defaultLocale, replacementRegex) {
	 ...
	 *		- (TODO) Replace the goog.getMsg call with the appropriate internationalized string, using the
	 *			arguments that were passed to the goog.getMsg call.
	 *
	 *	TODOS:
	 *	- use non-hardcoded input/output
	 *	- read in .xlf files and match up any dynamic variables in message definitions with the
	 *		arguments extracted via process above; generate actual language files
	 *	- clean up/abstract (use a stream.Transform for the goog.getMsg() reading)
	 *	- tests
	 *	- integrate with Webpack, or Grunt task wrapped around Webpack -- this should be used after bundle
	 *		generation and before uglification. It can't be added via a loader because that would require
	 *		using Webpack multi-compiler, which would take us back to square one (separate Webpack
	 *		compilation for each language)
	 *	- ?
	 */
